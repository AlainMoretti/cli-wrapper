...
# it interacts with the actual connection, and connects to the second proxy
# cli.py -r host -s subs.sfr_connection transparent_connection_to_2nd_proxy -v

def transparent_connection_to_2nd_proxy(args,c):
    c.sendline('USER')
    c.expect('Target machine address: ')
    c.sendline('HOST')
    c.expect('Target password: ')
    c.sendline('PASSWORD')
    c.expect('~ \$ ')
